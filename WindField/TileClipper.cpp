...
////////////GlobalRoadObject/////////////////////////////////////////
DEEPEYE_IMPLEMENT_TYPE_INFO(TileClipper,VisibleObject);
TileClipper::TileClipper()
	:resolution_(1)
	,render_view_idx_(0)
{
	is_reset = true;
	path_.Format("%splant\\WindField\\bin\\output.bin",resource_path_);
	reader_ = new WindFieldDataReader;
	MixedFileSystem::getSingleton().makeSureFileExistAndLatest(path_.GetString());
	is_read_data_ = reader_->ReadData(path_,wind_field_data_);
...
TileClipper::~TileClipper()
{
	...
	delete reader_;
	reader_ = NULL;
	delete particle_render_mgr_;
	particle_render_mgr_ = NULL;
}

TileClipper* TileClipper::GetInstance()
{
	static TileClipper* s_clipper = NULL;
	if (s_clipper == NULL)
	{
		s_clipper = new TileClipper;
	}
	return s_clipper;
}


bool TileClipper::ReadData(const CString& name)
{
	path_.Format("%splant\\WindField\\bin\\%s.bin",resource_path_,name);
	MixedFileSystem::getSingleton().makeSureFileExistAndLatest(path_.GetString());
	is_read_data_ = reader_->ReadData(path_,wind_field_data_);
	if (!is_read_data_)
	{
...
void TileClipper::Clip(RenderParameter& param, int intersection)
{
	if(!is_reset)
		return;
	if (!is_read_data_)
	...
	{
		RenderQueue* render_queue = framework_->m_renderQueueManager->GetRenderQueue(300);
...
void TileClipper::CalcBoundingBox()
{
	m_box.SetInfinite();
}

void TileClipper::ClipRecursive(RenderParameter& param,const CDEA_GlobeTile& tile, int intersection)
{
...
void TileClipper::PresentRenderTile(RenderParameter& param,const CDEA_GlobeTile& tile)
{
...
int TileClipper::FindMinRowNum() const
{
...
int TileClipper::FindMaxRowNum() const
{
	const VisibleTileContainer& visible_tile_list = visible_labeling_tiles_[render_view_idx_];
	const WindFieldData& wfd = wind_field_data_;
...
int TileClipper::FindMinColNum() const
{
	const VisibleTileContainer& visible_tile_list = visible_labeling_tiles_[render_view_idx_];
	const WindFieldData& wfd = wind_field_data_;
	int min_col_num = 0;
...
int TileClipper::FindMaxColNum() const
{
	const VisibleTileContainer& visible_tile_list = visible_labeling_tiles_[render_view_idx_];
	const WindFieldData& wfd = wind_field_data_;
...
Vector3d TileClipper::CalcNextPos(const Vector3d& cur_pos,Uint32 interval) const
{
